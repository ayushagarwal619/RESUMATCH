...
def render() -> None:
    st.markdown(
        ...
        <div style="margin-bottom: 2rem;">
            <h1 style="margin: 0; background: linear-gradient(135deg, var(--text-primary) 40%, var(--accent-primary) 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-weight: 800; font-size: 2.5rem; letter-spacing: -0.02em;">Analysis History</h1>
            <p style="margin: 6px 0 0 0; font-size: 1.05rem; color: var(--text-secondary); font-weight: 600;">Review your past resume evaluations.</p>
        </div>
        """),
    ...
    if not access_token:
        st.markdown(
            textwrap.dedent("""
            <div class="warn-card" style="padding: 1.5rem; margin-top: 1rem;">
                <div style="font-size: 1.8rem; margin-bottom: 0.5rem;">⚠️</div>
                <h3 style="color: var(--text-primary); margin: 0 0 0.5rem 0; font-weight: 800; font-size: 1.15rem;">Access Denied</h3>
    ...
    if not history:
        ...
        with col2:
            if st.button("🚀 Go to RESUMATCH Scorer", key="btn_go_scorer", use_container_width=True, type="primary"):
    ...
    for idx, entry in enumerate(history):
        ...
        with st.expander(title_str):
            ...
            if jd_comparison:
                st.markdown(
                    textwrap.dedent(f"""
                    <div style="margin-bottom: 1rem; font-weight: 700; font-size: 0.95rem; color: var(--accent-primary);">
