...
def display_recruiter_insights(analysis: Dict[str, Any]) -> None:
    ...
    with c1:
        ...
        html_inject(f"""
        <div class="glass-card" style="padding: 1.2rem; display: flex; flex-direction: column; justify-content: center; height: 110px; border: var(--border-soft); border-radius: 12px;">
    ...
    with c2:
        ...
        <div class="glass-card" style="padding: 1.2rem; display: flex; flex-direction: column; justify-content: center; height: 110px; border: var(--border-soft); border-radius: 12px;">
            ...
            <div class="shimmer-progress" style="height: 10px; background: var(--bg-secondary); border-radius: 4px; overflow: hidden; border: var(--border-soft);">
                <div class="shimmer-progress-fill" style="width: {completeness}%; background: var(--grad-primary); height: 100%;"></div>
            </div>
        </div>
        """)
    ...
    if red_flags:
        ...
        html_inject(f"""
        <div class="glass-card" style="border: 1.5px solid var(--color-danger); background: var(--high-impact-bg); padding: 1.2rem; border-radius: 12px;">
            <h4 style="color: var(--color-danger); margin: 0 0 8px 0; font-size: 1rem; font-weight: 800; display: flex; align-items: center; gap: 8px;">
    ...
    if conf:
        ...
        for k, v in conf.items():
            ...
            badges.append(f"<span style='display: inline-block; padding: 4px 10px; background: var(--bg-secondary); border: var(--border-soft); border-radius: 6px; font-size: 0.75rem; color: var(--text-secondary); margin-right: 8px; margin-bottom: 8px;'>{k.replace('_', ' ').title()}: <b>{pct}% Conf.</b></span>")
            
        html_inject(f"""
        <div style='display: flex; flex-wrap: wrap; align-items: center; border-top: var(--border-soft); padding-top: 1rem;'>
...
def display_results_dashboard(analysis: Dict[str, Any]) -> None:
    ...
    st.markdown("<div style='margin: 2rem 0; border-top: var(--border-soft);'></div>", unsafe_allow_html=True)
